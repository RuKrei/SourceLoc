...
h_freq: float = 50.0  # higher pass-band edge
...
freq_bands = dict(  # the frequency bands of interest for the analysis
    delta=(1, 4), theta=(4, 7), alpha=(8, 12), beta=(13, 29), gamma=(30, 50)
)
...
dip_times = {
    "min20ms": (
        -0.025,
        -0.020,
    ),  # Time points in Miliseconds for Equivalent current dipole fit
    "min15ms": (-0.019, -0.015),
    "min10ms": (-0.014, -0.010),
    "min5ms": (-0.009, -0.005),
    "peak": (-0.004, 0.000),
...
def main():
    ...
    dfc = Folderer.DerivativesFoldersCreator(
        BIDS_root=bids_root, extras_directory=extras_directory, subject=subject
    )
    ...
    logging.basicConfig(
        filename=logfile, filemode="w", format="\n%(levelname)s --> %(message)s"
    )
    ...
    if spacing not in ["ico4", "oct5", "oct6", "ico5"]:
        spacing = "oct6"
        print(
            'The desired spacing isn\'t allowed, typo?\n \
    ...
    sourcerer = Anatomist.SourceModeler(
        subjects_dir=dfc.fanat, subject=subject, spacing=spacing, n_jobs=n_jobs
    )
                ...
                if not "_prep" in rawfile:
                    ...
                    try:
                        ecg_projs, _ = mne.preprocessing.compute_proj_ecg(
                            raw, n_grad=n_grad, n_mag=n_mag, n_eeg=n_eeg, reject=None
                        )
                        raw.add_proj(ecg_projs, remove_existing=False)
                        fig = mne.viz.plot_projs_topomap(
                            ecg_projs, info=raw.info, show=False
                        )
        ...
        for event in concat_epochs.event_id.keys():
            ...
            if (
                ...
                try:
                    ...
                    cts_folder = os.path.join(
                        dfc.spikes, eventname, "custom_time_series"
                    )
                    ...
                    for m in source_loc_methods:
                        ...
                        if m == "dSPM":
                            ...
                            try:
                                ...
                                label = str(
                                    ject + " - " + eventname + " - Vector solution"
                                )
