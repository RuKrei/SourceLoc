...
class RawAnatomyProcessor:
    ...
    def _segment_hippocampal_subfields(self):
        ...
        hippofile = opj(
            self.FS_SUBJECTS_DIR, self.subject, "mri", "lh.hippoSfVolumes-T1.v21.txt"
        )
        if not os.path.isfile(hippofile):
            try:
                print(
                    f"Now running hippocampal segmentation for subject: {self.subject}\nThis might take some time"
                )
                hipposeg = "segmentHA_T1.sh " + self.subject
                self._run_shell_command(hipposeg)
            except Exception as e:
                print(
                    f"Something went wrong with the hippocampal segmentation! --> {e}"
                )
        else:
            print(
                f"Omitting hippocampal segmentation for subject {self.subject}, as it already exists"
            )
        ...
        if not os.path.isfile(srcfilename):
            try:
                src_vol = mne.setup_volume_source_space(
                    self.subject, pos=3.0, subjects_dir=self.subjects_dir, verbose=True
                )
                mne.write_source_spaces(
                    srcfilename, src_vol, overwrite=True, verbose=True
                )
            except Exception as e:
    ...
    def _make_cortex_source_space(self):
        ...
        if not os.path.isfile(srcfilename):
            try:
                ...
                print(
                    f"Failed to setup source space with spacing {self.spacing} \
                        for {self.subject} --> {e}"
                )
        ...
        if not os.path.isfile(bem_save_name):
            try:
                ...
                mne.write_bem_surfaces(
                    bem_save_name, bem, overwrite=True
                )  # , overwrite=True)
            except Exception as e:
                print(
